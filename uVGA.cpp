...
LED_ON;
	...
	end_of_display_line_dma_num_trigger = -1;
...
void uVGA::trigger_dma_channel(uvga_trigger_location_t location, short int dma_channel_num)
{
	...
	switch(location)
	{
		case UVGA_TRIGGER_LOCATION_END_OF_DISPLAY_LINE:
					end_of_display_line_dma_num_trigger = dma_channel_num;
					break;
...
void uVGA::clocks_init()
{
	...
	if(end_of_display_line_dma_num_trigger != -1)
		hftm->C[hsync_ftm_channel].SC |= FTM_CSC_DMA | FTM_CSC_CHIE;
...
void uVGA::clocks_start()
{
	...
	{
		// reconfigure user external DMA to trigger on hardware event generated by FTM channel X1+1
		*((volatile uint8_t *)&(DMAMUX0_CHCFG0) + end_of_display_line_dma_num_trigger) = DMAMUX_ENABLE | ftm_channel_to_dma_source(hsync_ftm, hsync_ftm_channel);
		edma->SERQ = end_of_display_line_dma_num_trigger;
	}
